...
const ChatbotPage = () => {
  ...
  // Keep related panel in sync with the most recent AI message that has related sections
  useEffect(() => {
    const lastAi = [...messages].reverse().find(m => m.role === 'ai' && m.related?.length > 0)
    setRelatedSections(lastAi ? lastAi.related : [])
  }, [messages])
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/chat`,
        { message: userMessage, language: i18n.language },
        {
          headers: {
      ...
      setMessages(prev => [...prev, { role: 'ai', text: aiReply, confidence: confidence, related: related }])
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-white dark:bg-[#1e1917] transition-colors duration-300 relative overflow-hidden">
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <div> 
                <h2>{t('disclaimer:')}</h2>
                <p>
                  {t('This AI assistant provides general legal information for educational and informational purposes only. It is not a lawyer and does not provide legal advice. Laws may change and legal outcomes depend on specific facts and circumstances. Please consult a qualified lawyer or licensed legal professional for advice regarding your situation.')}
                </p>
                </div>
            </div>
          ) : (
      ...
      {/* RELATED LAWS PANEL — right side, only shown when there are results */}
      {relatedSections.length > 0 && (
        <div className="hidden lg:flex flex-col w-72 xl:w-80 h-full border-l border-gray-200 dark:border-[#4a3b35] bg-white dark:bg-[#1e1917] overflow-hidden flex-shrink-0">
          
          {/* Panel header */}
          <div className="px-4 py-3 border-b border-gray-200 dark:border-[#4a3b35] bg-gray-50 dark:bg-[#302623]">
            <div className="flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-blue-600 dark:text-[#c69f6f] flex-shrink-0">
                <path d="M11.25 4.533A9.707 9.707 0 006 3a9.735 9.735 0 00-3.25.555.75.75 0 00-.5.707v14.25a.75.75 0 001 .707A8.237 8.237 0 016 18.75c1.995 0 3.823.707 5.25 1.886V4.533zM12.75 20.636A8.214 8.214 0 0118 18.75c.966 0 1.89.166 2.75.47a.75.75 0 001-.708V4.262a.75.75 0 00-.5-.707A9.735 9.735 0 0018 3a9.707 9.707 0 00-5.25 1.533v16.103z" />
              </svg>
              <h2 className="text-sm font-semibold text-gray-700 dark:text-[#f5e6d3]">
                Related Laws
              </h2>
            </div>
            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 ml-6">
              Sections mathematically similar to your results
            </p>
          </div>

          {/* Panel items */}
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {relatedSections.map((item, idx) => (
              <div
                key={idx}
                className="p-3 rounded-xl border border-gray-200 dark:border-[#5d4a42] bg-gray-50 dark:bg-[#302623] hover:bg-gray-100 dark:hover:bg-[#3d312d] transition-colors cursor-default group"
              >
                {/* Law + section badge */}
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <span className="text-[10px] font-bold uppercase tracking-wide text-blue-600 dark:text-[#c69f6f] leading-tight">
                    {item.law.replace(/_/g, " ")}
                  </span>
                ...
                {/* Title */}
                {item.title && (
                  <p className="text-xs font-medium text-gray-700 dark:text-[#f5e6d3] mb-1 leading-snug line-clamp-2">
                    {item.title}
                  </p>
                )}
